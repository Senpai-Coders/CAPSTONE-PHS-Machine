...
const cam_var1 = [
  ...
  },
  {
    ...
    stream_url : "https://www.anolytics.ai/wp-content/uploads/2020/01/2020-01-14.gif"
  },
  {
...
const cam_var2 = [
  {
    name: "Thermal",
    stream_url: "https://i.pinimg.com/originals/22/23/6f/22236f3e1e301b5ad39e575067e740d6.gif",
  },
  {
    name: "Thermal + Annotated Ear",
    stream_url: "http://178.194.8.37:9000/mjpg/video.mjpg?timestamp=1652597508697",
  },{
      name : "Penguin Desu",
...
const cameras = () => {
  ...
  const [thermcamstat, setThermCamStat] = useState(false);

  const [view1, setView1] = useState(cam_var1[0]);
  const [view2, setView2] = useState(cam_var2[0]);
  ...
  return (
    <div className="mt-5 mb-8 gap-4 grid grid-cols-1 sm:grid-cols-2">
      ...
      <div className="">
        <div className="flex flex-wrap bg-stone-900/5 rounded-md px-2 justify-between items-center mb-3">
          <div className="w-72">
            <Listbox value={view2} onChange={setView2}>
              <div className="relative">
                <Listbox.Button className="relative bg-base-100/5 w-full cursor-default rounded-lg py-2 pl-3 pr-10 text-left focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                  <span className="block truncate">{view2.name}</span>
                  <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                    <HiOutlineSelector
                      className="h-5 w-5 "
                      aria-hidden="true"
          ...
          <div className="flex justify-end items-center space-x-3">
            <label className="label cursor-pointer">
              <input
                ...
                checked={thermcamstat}
                onChange={() => setThermCamStat(!thermcamstat)}
              />
            </label>
            <div className="relative">
              <p
                className={` bg-stone-700 text-white px-3 py-1 rounded-xl text-xs border ${
                  thermcamstat ? "border-rose-400" : "border-stone-500"
                } `}
              >
                {thermcamstat ? "LIVE" : "disconnected"}
              </p>
              {thermcamstat && (
                <span className="animate-ping absolute w-2 h-2 rounded-full -top-0 right-0 bg-rose-400 opacity-95"></span>
              )}
        ...
        <div className="relative">
          {thermcamstat ? (
            <>
              <img
                className="rounded-lg w-full aspect-video"
                src={view2.stream_url}
              ></img>
            </>
          ) : (
            <div className="rounded-lg border w-full flex items-center justify-center aspect-video">
              <p className="font-lato tracking-widest">disconnected</p>
            </div>
          )}
        </div>
      </div>
