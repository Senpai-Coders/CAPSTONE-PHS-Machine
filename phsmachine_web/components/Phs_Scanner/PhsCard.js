...
const PhsCard = ({ phs_data, showConnect, title }) => {
  ...
  useEffect(() => {
    ...
    let Size = mbToGB(bytesToMegaBytes(phs_data.storage.size)),
      Free = mbToGB(bytesToMegaBytes(phs_data.storage.free)),
      Used = Size - Free;
  ...
  return (
    <div className="shadow-lg md:mr-2 py-5 px-6 md:w-6/12 lg:w-4/12 card bg-neutral text-neutral-content">
      ...
      <div className="mx-2">
        ...
        {showConnect && (
          <a
            className="btn gap-4 mt-4 btn-block btn-secondary"
            href={phs_data.url}
          >
            Connect{" "}
          </a>
